...
DEFAULT_DB_PATH = REPO_ROOT / "backend" / "app.db"
SESSION_DAYS = 30
...
def connect() -> sqlite3.Connection:
    db_path = Path(os.environ.get("IR_APP_DB_PATH", DEFAULT_DB_PATH))
    ...
    with connect() as conn:
        conn.execute(
            ...
            INSERT INTO checkins (
                id, user_id, checkin_date, data_json, model_payload_json,
                risk_result_json, created_at, updated_at
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
